...
##load data
```{r}
metadata.csv <- read.csv("PRJNA558500MetaData.csv")

load_samples <- function(df) {
  
  sample_names <- rownames(df)
  
  retro_files <- file.path(paste("Raw Data/telescope/", sep = ""))
  
retro_counts <- load_telescope_reports("Raw Data/telescope/SRR9898939/SRR9898939.tsv"
